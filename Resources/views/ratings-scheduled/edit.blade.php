...
<div>
    <div class="content-header mb-3">
        <h1>Rediger Scheduled Rating</h1>
    </div>

    <form method="POST" action="{{ route('promatch.ratings-scheduled.update', $taskRatingScheduled->id) }}">
        @csrf
        @method('PUT')
                ...
                <tr>
                    ...
                    <td>
                        <input type="datetime-local" 
                               ...
                               name="scheduled_at" 
                               value="{{ old('scheduled_at', $taskRatingScheduled->scheduled_at ? $taskRatingScheduled->scheduled_at->format('Y-m-d\TH:i') : '') }}"
                               required>
                        @error('scheduled_at')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <tr>
                    ...
                    <td>
                        <input type="datetime-local" 
                               ...
                               name="completed_at" 
                               value="{{ old('completed_at', $taskRatingScheduled->completed_at ? $taskRatingScheduled->completed_at->format('Y-m-d\TH:i') : '') }}">
                        @error('completed_at')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </td>
